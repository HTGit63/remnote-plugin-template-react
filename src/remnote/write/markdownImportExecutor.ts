...
import { markdownImportOutputTextFromTree, normalizeMarkdownImportArgs, parseMarkdownImportPlan, verifyMarkdownSourceFidelity } from '../../../shared/bridge/markdown-importer';
...
export function rememberMarkdownImportResult(idempotencyKey: string, result: CreateOrReplaceNoteFromMarkdownResult) {
...
export async function findDirectChildByPlainText(plugin: RNPlugin, parentId: string, plainText: string): Promise<Rem | null> {
...
export async function collectPlainTextForRemIds(plugin: RNPlugin, remIds: readonly string[]): Promise<string> {
  const parts: string[] = [];
...
export function markdownImportPlanSummary(plan: ReturnType<typeof parseMarkdownImportPlan>): CreateOrReplaceNoteFromMarkdownResult['plan'] {
  return {
      ...
      if (duplicate && normalized.duplicatePolicy === 'skip') {
        const result: CreateOrReplaceNoteFromMarkdownResult = {
          ...
          verification: normalized.safetyOptions.verifyAfterWrite
            ? verifyMarkdownSourceFidelity(plan.sourceSnippets, await getRemPlainString(plugin, duplicate), normalized.fidelityOptions, plan.stats)
            : undefined,
        };
