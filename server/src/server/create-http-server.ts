  ...
  function writeUnknownToolCall(body: unknown, res: ServerResponse, toolProfile: ToolProfile): boolean {
    ...
    writeJson(res, 200, {
      ...
      result: {
        ...
        structuredContent: {
          ok: false,
          status: blockedByTier ? 'BLOCKED_BY_PROFILE' : 'FAIL',
          toolName: name,
          operationId: typeof request.id === 'string' || typeof request.id === 'number' ? String(request.id) : 'unknown',
          ...
          verification: {
            listedInActiveProfile: false,
            blockedByProfile: blockedByTier,
          },
          phaseDurations: {},
          warnings: blockedByTier ? [`Tool is hidden by active profile "${toolProfile}".`] : [],
          error: {
            code: 'UNKNOWN_TOOL',
