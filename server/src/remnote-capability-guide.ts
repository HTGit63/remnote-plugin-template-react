...
const GUIDE_BLOCKS: RemnoteCapabilityGuideBlock[] = [
  ...
  },
  {
    ...
    facts: [
      ...
      'Preferred high-level note writers are `create_or_replace_note_from_markdown` for Markdown/source notes and `create_polished_note_tree` / `apply_structured_note_batch` for structured JSON; they support idempotency, verification, styled nested nodes, and math.',
    ...
    bridgeUse: [
      'For prepared Markdown/source notes, first read context, then call `create_or_replace_note_from_markdown` once with the whole source. For prebuilt JSON trees, call `create_polished_note_tree` or `apply_structured_note_batch` once.',
