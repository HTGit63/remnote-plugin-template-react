...
export const REMNOTE_CAPABILITY_GUIDE_VERSION = '2026-07-20.hosted-image-media';
...
const GUIDE_BLOCKS: RemnoteCapabilityGuideBlock[] = [
  ...
  },
  {
    ...
    facts: [
      ...
      'Authenticated ChatGPT image files can be downloaded once, validated by byte signature, stored persistently in PostgreSQL, and exposed through an opaque immutable HTTPS asset URL.',
    ],
    bridgeUse: [
      ...
      'Hosted image URLs are intentionally public and unguessable because RemNote must fetch them; do not use this path for secrets.',
    ],
  },
  {
    section: 'flashcards',
  ...
  },
  {
    ...
    facts: [
      ...
      'Preferred delete tool is `delete_rem_by_id`. It defaults to dryRun, requires prior dry-run plus exact ID/title/parent/ancestor guards for real deletion, and verifies the Rem cannot be read afterward.',
      'Legacy focus/selection delete tools are removed. Only guarded explicit-ID deletion is available.',
      'Preferred high-level note writer is `create_or_replace_note_from_markdown` for Markdown/source notes. Use the resumable bulk import job tools for long chapters.',
      '`create_note_from_markdown_tree`, `create_styled_rem_tree`, `create_polished_note_tree`, and `apply_structured_note_batch` are fallback or higher-tier tools. Do not use them as the normal full-note path.',
    ],
    bridgeUse: [
      'For prepared Markdown/source notes, first read context, then call `create_or_replace_note_from_markdown` once for bounded notes or the plan/start/run/resume/verify bulk import job flow for long chapters.',
      ...
      'Apply styling only after content readback passes. For styling existing Rems, use `apply_style_plan`; for checking design after a write, use `verify_note_design`.',
      'For deletion, call `delete_rem_by_id` with `dryRun: true` and idempotencyKey, inspect breadcrumbs/childCount, then retry with `dryRun: false`, the same idempotencyKey, confirmTitle, expectedParentId, expectedAncestorId, and requirePriorDryRun=true.',
...
export function getRemnoteCapabilityGuide(section: RemnoteCapabilityGuideSection = 'all') {
  ...
  return {
    ...
    recommendedMarkdownImportTool: 'create_or_replace_note_from_markdown',
    recommendedStructuredNoteTool: 'create_or_replace_note_from_markdown',
    recommendedAtomicBatchTool: 'plan/start/run/resume/verify_note_import_job',
    recommendedStyleTool: 'apply_style_plan',
    recommendedVerificationTool: 'verify_note_design',
    preferredDeleteTool: 'delete_rem_by_id',
    fallbackDeveloperTools: ['create_note_from_markdown_tree', 'create_styled_rem_tree', 'create_polished_note_tree', 'apply_structured_note_batch'],
    ...
    preferredMediaTools: ['insert_image_from_file', 'insert_image_from_url', 'insert_audio_from_url', 'insert_video_from_url'],
    mediaBoundary: 'ChatGPT image files are validated and durably hosted; all native RemNote media remains URL-backed',
  };
}
