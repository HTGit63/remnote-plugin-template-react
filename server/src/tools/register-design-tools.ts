...
const CARD_LIMIT_SCHEMA = z.number().int().min(1).max(100).default(40);
...
export function registerDesignTemplateTools({ registerTool, callPlugin }: ToolRegistrationContext): void {
  ...
  registerTool(
    'list_note_design_templates',
    {
      ...
      inputSchema: z.object({
        includeRules: z.boolean().default(false).optional(),
      }),
      outputSchema: BRIDGE_TOOL_OUTPUT_SCHEMA,
  ...
  registerTool(
    'preview_note_design_plan',
    {
      ...
      inputSchema: z.object({
        ...
        mode: z.enum(['create', 'append', 'replace_children', 'repair']).default('create').optional(),
        rules: DESIGN_TEMPLATE_RULES_SCHEMA.optional(),
      }),
      outputSchema: BRIDGE_TOOL_OUTPUT_SCHEMA,
  ...
  registerTool(
    'create_designed_note_tree',
    {
      ...
      inputSchema: z.object({
        ...
        templateId: TEMPLATE_ID_SCHEMA.optional(),
        writingMode: z.enum(['markdown', 'styled_tree']).default('markdown').optional(),
        ...
        maxDepth: TREE_DEPTH_SCHEMA.optional(),
        maxNodeCount: MAX_TREE_NODE_COUNT_SCHEMA.optional(),
      }),
      outputSchema: BRIDGE_TOOL_OUTPUT_SCHEMA,
